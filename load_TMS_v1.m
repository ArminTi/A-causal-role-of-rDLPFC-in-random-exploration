...
fname = 'TMSanalysis_192_data.csv';
...
hdr = textscan(fid,...
    ...
    1, 'delimiter', ',');
data = textscan(fid,...
    '%s%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f', ...
    'delimiter', ',');
fclose(fid);
...
for sn = 1:max(data{2})
    ...
    sub(sn).subjectID   = num2str(data{2}(ind(1)));
    sub(sn).order       = data{3}(ind(1));
    sub(sn).age         = data{4}(ind(1));
    sub(sn).iswoman     = data{5}(ind(1));
    
    sub(sn).game        = data{7}(ind);
    sub(sn).gameLength  = data{8}(ind);
    sub(sn).uc          = data{9}(ind);
    sub(sn).m1          = data{10}(ind);
    sub(sn).m2          = data{11}(ind);
